...
# Optional C++ backend
try:
    import cpp_negf  # built from cpp/native via pybind11
    _cpp_loaded = True
except Exception:
    _cpp_loaded = False

# Allow disabling C++ fast path from environment for A/B benchmarking
CPP_NEGF_AVAILABLE = _cpp_loaded and (os.getenv("NEGFSW_FORCE_PY", "") == "")
...
class NEGFSolver:
    ...
    def surface_greens_function(self, energy: float, lead_idx: int) -> np.ndarray:
        ...
        # Fast path via C++ if available
        if CPP_NEGF_AVAILABLE:
            try:
                return cpp_negf.surface_gf(H00.astype(np.complex128), H01.astype(np.complex128), float(energy), float(self.eta))
            except Exception:
                pass
    ...
    def self_energy(self, energy: float, lead_idx: int) -> np.ndarray:
        ...
        """
        try:
            # C++ fast path if available
            if CPP_NEGF_AVAILABLE:
                try:
                    lead = self._lead_info[lead_idx]
                    return cpp_negf.self_energy_from_lead(lead['H00'].astype(np.complex128), lead['H01'].astype(np.complex128),
                                                          self.V_couplings[lead_idx].astype(np.complex128), float(energy), float(self.eta))
                except Exception:
                    pass
    ...
    def transmission(self, energy: float, lead_i: int = 0, lead_j: int = 1) -> float:
        ...
        # Fast path: C++ using lead blocks directly if available
        if CPP_NEGF_AVAILABLE and self.n_leads >= 2:
            try:
                H = np.asarray(self.H_device, dtype=np.complex128)
                L = self._lead_info[lead_i]
                R = self._lead_info[lead_j]
                V_L = self.V_couplings[lead_i]
                V_R = self.V_couplings[lead_j]
                T = cpp_negf.transmission_from_leads(H,
                                                     L['H00'].astype(np.complex128), L['H01'].astype(np.complex128), V_L.astype(np.complex128),
                                                     R['H00'].astype(np.complex128), R['H01'].astype(np.complex128), V_R.astype(np.complex128),
                                                     float(energy), float(self.eta))
                return float(T)
            except Exception:
                pass
        ...
        return float(np.real(np.trace(T_matrix)))
    ...
    def finite_bias_current_cpp(self,
                                mu_left: float,
                                mu_right: float,
                                temperatureK: float,
                                E_center: float,
                                E_span: float = 0.2,
                                NE: int = 121) -> Optional[float]:
        """Compute I(V) using the C++ backend if available. Returns None on failure."""
        if not CPP_NEGF_AVAILABLE:
            return None
        try:
            if self.n_leads >= 2:
                H = np.asarray(self.H_device, dtype=np.complex128)
                L = self._lead_info[0]; R = self._lead_info[1]
                V_L = self.V_couplings[0]; V_R = self.V_couplings[1]
                I = cpp_negf.finite_bias_current_from_leads(H,
                                                            L['H00'].astype(np.complex128), L['H01'].astype(np.complex128), V_L.astype(np.complex128),
                                                            R['H00'].astype(np.complex128), R['H01'].astype(np.complex128), V_R.astype(np.complex128),
                                                            float(mu_left), float(mu_right), float(temperatureK),
                                                            float(E_center), float(E_span), int(NE), float(self.eta))
                return float(I)
            else:
                # Fallback to existing C++ function using Sigmas at E_center (approx)
                Sigma_L = self.self_energy(E_center, 0)
                Sigma_R = self.self_energy(E_center, 1 if self.n_leads>1 else 0)
                H = np.asarray(self.H_device, dtype=np.complex128)
                I_arr = cpp_negf.finite_bias_current(H, [Sigma_L.astype(np.complex128), Sigma_R.astype(np.complex128)],
                                                     float(mu_left), float(mu_right), float(temperatureK),
                                                     float(E_center), float(E_span), int(NE))
                return float(np.array(I_arr)[0])
        except Exception:
            return None
