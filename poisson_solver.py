...
class PoissonSolverFEM:
    ...
    def _build_element_matrices(self):
        ...
        for tri_nodes in self.triangles:
            ...
            if area < 1e-12:  # Degenerate triangle
                ...
                # For Poisson: ∇·(ε∇φ) = -ρ → K should be scaled by ε₀εᵣ
                B = np.array([[b1, b2, b3], [c1, c2, c3]]) / (2 * area)
                K_elem = (B.T @ B) * area * (self.eps0 * self.epsilon)
    ...
    def _solve_2d_fem(self, charge_density: np.ndarray, 
        ...
        # Convert charge density (electrons/site) → surface charge density ρ_2d (C/m²)
        # Use actual mesh geometry: per-node control area from triangulation
        n = self.n_sites
        node_area = np.zeros(n, dtype=float)
        for elem_idx, tri_nodes in enumerate(self.triangles):
            area = self.areas[elem_idx]
            # Distribute one third of triangle area to each vertex (barycentric control area)
            share = area / 3.0
            node_area[tri_nodes[0]] += share
            node_area[tri_nodes[1]] += share
            node_area[tri_nodes[2]] += share
        # Avoid division by zero for isolated/degenerate nodes
        node_area = np.maximum(node_area, 1e-24)
        rho_2d = (charge_density * self.e) / node_area  # C/m² per node

        # Assemble global system: K includes (ε₀εᵣ), f has -ρ
        ...
        # If non-finite, attempt diagonal regularization retries
        if not np.all(np.isfinite(phi_interior)):
            diag_mean = float(np.mean(K_II.diagonal())) if K_II.shape[0] > 0 else 1.0
            base = diag_mean if np.isfinite(diag_mean) and diag_mean > 0 else 1.0
            for lam in (1e-12, 1e-10, 1e-8, 1e-6):
                try:
                    K_reg = K_II + (lam * base) * sp.eye(K_II.shape[0], format='csr')
                    phi_try = self._solve_cpu(K_reg, f_I)
                    if np.all(np.isfinite(phi_try)):
                        phi_interior = phi_try
                        warnings.warn(f"FEM needed regularization lambda={lam*base:.2e}")
                        break
                except Exception:
                    continue
            else:
                # Final attempt with CG directly on regularized system
                K_reg = K_II + (1e-4 * base) * sp.eye(K_II.shape[0], format='csr')
                try:
                    phi_try, info = spla.cg(K_reg, f_I, maxiter=1000, rtol=1e-8)
                except TypeError:
                    try:
                        phi_try, info = spla.cg(K_reg, f_I, maxiter=1000, tol=1e-8)
                    except TypeError:
                        phi_try, info = spla.cg(K_reg, f_I, maxiter=1000)
                if info == 0 and np.all(np.isfinite(phi_try)):
                    phi_interior = phi_try
                else:
                    raise RuntimeError("FEM 2D solve produced non-finite values after regularization attempts")
        ...
        # Remove mean drift and re-anchor boundary nodes exactly
        phi -= np.mean(phi)
        phi[left_nodes] = V_left
        phi[right_nodes] = V_right
    ...
    def _assemble_global_matrix_2d(self, charge_density: np.ndarray):
        ...
        for elem_idx, tri_nodes in enumerate(self.triangles):
            ...
            # After assembly: K*phi = f
            # Weak form: ∫ε₀εᵣ ∇φ·∇N dA = -∫ρN dA
            # So f = -∫ρN dA, with ρ in C/m²
            for i in range(3):
                rho_avg = np.mean(charge_density[tri_nodes])
                # Integration: ∫ρN_i dA ≈ ρ_avg * area/3
                # Note the negative sign from Poisson equation!
                f[tri_nodes[i]] -= rho_avg * area / 3.0
    ...
    def _solve_cpu(self, K, f):
        ...
        try:
            # Add tiny diagonal regularization to improve conditioning
            n = K.shape[0]
            if n > 0:
                diag_scale = float(np.mean(K.diagonal()))
                if not np.isfinite(diag_scale) or diag_scale <= 0:
                    diag_scale = 1.0
                K_reg = K + (1e-12 * diag_scale) * sp.eye(n, format='csr')
            else:
                K_reg = K

            # Quietly handle MatrixRankWarning by catching it, not spamming logs
            try:
                try:
                    from scipy.sparse.linalg import MatrixRankWarning  # type: ignore
                except Exception:
                    try:
                        from scipy.sparse.linalg.dsolve.linsolve import MatrixRankWarning  # type: ignore
                    except Exception:
                        MatrixRankWarning = Warning  # type: ignore
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore", category=MatrixRankWarning)
                    phi = spla.spsolve(K_reg, f)
            except Exception:
                # If MatrixRankWarning isn't available or other issue, try direct solve
                phi = spla.spsolve(K_reg, f)
            ...
            # Fall back to CG without noisy warnings
            try:
                phi, info = spla.cg(K, f, maxiter=1000, rtol=1e-8)
            except TypeError:
                try:
                    phi, info = spla.cg(K, f, maxiter=1000, tol=1e-8)
                except TypeError:
                    phi, info = spla.cg(K, f, maxiter=1000)
            return phi
    ...
    def _solve_gpu(self, K, f):
        ...
        try:
            ...
            try:
                phi_gpu, info = cpspla.cg(K_gpu, f_gpu, maxiter=1000, rtol=1e-8)
            except TypeError:
                try:
                    phi_gpu, info = cpspla.cg(K_gpu, f_gpu, maxiter=1000, tol=1e-8)
                except TypeError:
                    phi_gpu, info = cpspla.cg(K_gpu, f_gpu, maxiter=1000)
...
class PoissonSolverGraphene:
    ...
    def solve_graphene(self, charge_density_sites: np.ndarray,
                      ...
                      lead_potentials: Optional[Tuple[float, float]] = None,
        ...
        # Sanitize input charge
        if not np.all(np.isfinite(charge_density_sites)):
            charge_density_sites = np.nan_to_num(charge_density_sites, nan=0.0, posinf=0.0, neginf=0.0)
        print(f"DEBUG: Poisson input charge range: [{np.min(charge_density_sites):.3e}, {np.max(charge_density_sites):.3e}]")

        # Heuristic: decide if FEM is viable for this geometry (used when method is None or 'auto')
        fem_viable = False
        if self.fem_available:
            try:
                # Check interior coverage (avoid cases with almost no interior nodes)
                x_coords = self.lattice_sites[:, 0]
                x_min, x_max = float(np.min(x_coords)), float(np.max(x_coords))
                tol = (x_max - x_min) * 0.01
                left_nodes = np.where(x_coords <= x_min + tol)[0]
                right_nodes = np.where(x_coords >= x_max - tol)[0]
                boundary_nodes = np.unique(np.concatenate([left_nodes, right_nodes]))
                interior_count = self.n_sites - boundary_nodes.size
                # Mesh quality based on triangle area distribution, if available
                areas = getattr(self.fem_solver, 'areas', [])
                if isinstance(areas, list):
                    areas = np.array(areas, dtype=float) if len(areas) else np.array([])
                small_area_frac = 0.0
                area_ok = True
                if areas.size:
                    med_area = np.median(areas[areas > 0]) if np.any(areas > 0) else 0.0
                    small_area_frac = float(np.mean(areas < max(1e-24, 1e-8 * med_area)))
                    area_ok = med_area > 0 and small_area_frac < 0.2
                fem_viable = (interior_count >= max(5, int(0.1 * self.n_sites))) and area_ok
            except Exception:
                fem_viable = False

        # Interpret method selection
        method = (method or 'auto').lower()

        # Try FEM solver first when viable, or when explicitly selected
        force_fem = (method == 'fem')
        if self.fem_available and (fem_viable or force_fem) and method in ('auto', 'fem'):
            try:
                boundary_conditions = {
                ...
                # Validate FEM output; fall back if non-finite or extreme
                if not np.all(np.isfinite(phi)):
                    raise RuntimeError("FEM returned non-finite potential; using graph-Laplacian fallback")
                span = float(np.ptp(phi)) if phi.size else 0.0
                bias_scale = max(abs(lead_potentials[0]) + abs(lead_potentials[1]), 1e-6)
                if span > 1e5 * bias_scale:
                    raise RuntimeError(f"FEM potential span {span:.3e} unphysically large; using graph-Laplacian")
        ...
        # Fallback to graph-Laplacian solve (with improved scaling) if method allows
        ...
        # Set up boundary conditions for finite bias on regular grid
