    ...
    try:
        ...
        try:
            if getattr(args, 'use_scf', False):
                G_sw, T_sw = scf_conductance_wrapper(fsys_sw, args.EF, params_sw, bias_voltage=bias, temperature=tempK)
        ...
        if include_pristine and fsys_pr is not None:
            ...
            try:
                if getattr(args, 'use_scf', False):
                    G_pr, T_pr = scf_conductance_wrapper(fsys_pr, args.EF, params_pr, bias_voltage=bias, temperature=tempK)
                elif getattr(args, 'use_negf', False):
...
def compute_multi_bias_iv_parallel(args, bias_voltages, include_pristine=True, max_workers=None):
    """Parallel version: compute I–V for multiple biases using process pool.
    ...
    results = {
        ...
        'pristine_system': {'currents': [], 'conductances': [], 'transmissions': []} if include_pristine else None
    }

    with ProcessPoolExecutor(max_workers=max_workers) as ex:
        futures = {ex.submit(_parallel_bias_worker, args_dict, b, include_pristine): b for b in bias_voltages}
...
def build_SW_system(args):
    ...
    # --- Lherbier mode vs proxy ---
    if args.lherbier_sw:
        # (i) No hop scaling; keep pristine |t| on all bonds
        for hop in lat.neighbors():
            sys[hop] = (lambda val: (lambda s1, s2: -val * I2))(args.t)
        
        # (ii) Onsite-only defect potential on atoms in the SW core and shells
        core_sites = [A0, A1, B0, B1]
        
        # Build neighbor shells by graph distance
        from collections import deque, defaultdict
        dist = {site: np.inf for site in sys.sites()}
        for cs in core_sites:
            dist[cs] = 0
        q = deque(core_sites)
        adj = defaultdict(list)
        for (s1, s2) in sys.hoppings():
            adj[s1].append(s2)
            adj[s2].append(s1)
        while q:
            s = q.popleft()
        ...
        shell0 = set(core_sites)
        shell1 = {s for s,d in dist.items() if d == 1}
        shell2 = {s for s,d in dist.items() if d == 2}
        
        # Apply onsite shifts
        def make_ons(dE):
            return (lambda val: (lambda s: val * I2))(dE)
        ons0 = make_ons(args.sw_ons_core)
        for s in shell0:
            sys[s] = ons0
        if abs(args.sw_ons_shell1) > 0:
            ons1 = make_ons(args.sw_ons_shell1)
            for s in shell1:
                sys[s] = ons1
        if abs(args.sw_ons_shell2) > 0:
            ons2 = make_ons(args.sw_ons_shell2)
            for s in shell2:
                sys[s] = ons2
    else:
        # Legacy proxy: small onsite tweak on the shortest (η^2) pair + hop scaling on bonds touching those two atoms
    ...
    def Tb_impurity_onsite(site, X=0, g=2.0, Bx=0, By=0, Bz=0, 
                          eps_sw=0, alpha1_sw=0, alpha2_sw=0,
                          electrostatic_potential=None, site_index_map=None, **kwargs):
        """
        Tb3+ ground state is 8S_7/2, but crystal field splits this into 
        ...
        crystal_field_gap = 0.055  # ~20 meV typical for Tb³⁺ in graphene environment
        
        # Stark shifts (different for each level due to different J quantum numbers)
        stark_linear = 0.0  # accounted via alpha1_sw# Enhanced linear Stark coefficient for 4f electrons  
        stark_quad = 0.0  # accounted via alpha2_sw# Enhanced quadratic Stark term
        ...
        stark_mixing = 1e-3 * X  # small mixing# Enhanced field-induced mixing between J levels
    ...
    # Add Tb impurity at the center of SW defect
    core = As + Bs
    pos_tb = np.mean([np.array(s.pos) for s in core], axis=0)
    tb = Tb_sub(*pos_tb)
    sys[tb] = Tb_impurity_onsite
    
    # Coupling between Tb and graphene sites
    ...
    attach_leads(sys, lat, A, B, args.W, args.t)
...
def build_large_SW_system(args):
    ...
    for i, sw_center in enumerate(sw_centers):
        ...
        def tb_onsite_sw_large(site,
            ...
            crystal_field_gap = 0.055  # ~20 meV

            # Enhanced Stark shifts for realistic sensitivity
            stark_linear = 0.0  # accounted via alpha1_sw# Enhanced linear coefficient
            stark_quad = 0.0  # accounted via alpha2_sw# Enhanced quadratic term
            ...
            # Off-diagonal mixing
            stark_mixing = 1e-3 * X  # small mixing# Enhanced field-induced mixing
...
def parse_args():
    ...
    p.add_argument("--L",type=float,default=20.0)  # Increased default length
    ...
    p.add_argument("--Vimp_eta2",type=float,default=0.2)  # MCMC optimized
    p.add_argument("--Vimp_side",type=float,default=0.2)
    p.add_argument("--Vimp_pr",type=float,default=0.2)  # MCMC optimized
    p.add_argument("--dE_on_eta2",type=float,default=0.03)
    p.add_argument("--hop_scale_eta2",type=float,default=0.90)

    p.add_argument("--lherbier_sw", action="store_true", help="Use Lherbier et al. (2012) SW TB: rotate bond + only onsite shifts on core/shell atoms (no hop scaling)", default=True)
    p.add_argument("--sw_ons_core", type=float, default=0.12, help="Onsite shift (eV) for the four core atoms of the SW defect (DFT-fitted order of 0.1 eV)")
    p.add_argument("--sw_ons_shell1", type=float, default=0.06, help="Onsite shift (eV) for first-neighbor shell around SW core")
    p.add_argument("--sw_ons_shell2", type=float, default=0.00, help="Onsite shift (eV) for second-neighbor shell (default 0 per Lherbier emphasis on core)")
    p.add_argument("--eps_sw",type=float,default=1.5)  # MCMC optimized
    p.add_argument("--alpha1_sw",type=float,default=2e-5)   # 10 meV per V/Å (realistic for defect states)  
    p.add_argument("--alpha2_sw",type=float,default=0.5) # Quadratic coupling for field-induced distortion
    p.add_argument("--eps_pr",type=float,default=1.5)  # MCMC optimized
    p.add_argument("--alpha1_pr",type=float,default=1e-5)  # Smaller coupling for pristine (no defect enhancement)
    p.add_argument("--alpha2_pr",type=float,default=0.2) # Weaker quadratic term for pristine
    # Magnetic field parameters (MCMC optimized)
    p.add_argument("--g",type=float,default=2.0)  # MCMC optimized g-factor
...
def main():
    ...
    if args.plot:
        ...
        plot_transmission_vs_energy(fsys_sw, args, ax_energy)
