...
class NEGFSolver:
    ...
    def surface_greens_function(self, energy: float, lead_idx: int) -> np.ndarray:
        ...
        # If H01 is not square, we need to handle it carefully
        if H01.shape[1] != H00.shape[0]:
            # Try padding approach first (less destructive than truncation)
        ...
        # Initial values for recursive algorithm
        # Now H01_trunc is guaranteed to be square
    ...
    def self_energy(self, energy: float, lead_idx: int) -> np.ndarray:
        ...
        try:
            ...
            # Check dimensions and handle mismatches carefully
            if V_coupling.shape[0] != g_s.shape[0]:
                n_lead_g = g_s.shape[0]  
                n_lead_v = V_coupling.shape[0] 
                
                # Only try to fix if mismatch is small (likely due to padding/truncation)
                if abs(n_lead_g - n_lead_v) <= n_lead_v // 2:  # Allow up to 50% difference
                    if n_lead_g > n_lead_v:
                        # Pad coupling matrix
                        V_padded = np.zeros((n_lead_g, V_coupling.shape[1]), dtype=V_coupling.dtype)
                        V_padded[:n_lead_v, :] = V_coupling
                        Sigma = V_padded.T.conj() @ g_s @ V_padded
                    else:
                        # Truncate coupling matrix
                        V_truncated = V_coupling[:n_lead_g, :]
                        Sigma = V_truncated.T.conj() @ g_s @ V_truncated
                else:
                    # Large mismatch - raise error rather than silently giving wrong physics
                    raise ValueError(f"Large dimension mismatch in self-energy: V_coupling {V_coupling.shape} vs g_s {g_s.shape}")
            else:
...
def extract_kwant_matrices(fsys, energy: float, leads_kwant_supported: list = None, params: dict = None) -> Tuple[np.ndarray, list, list]:
    ...
    required_params = {
        'eps_sw', 'alpha1_sw', 'alpha2_sw', 'eps_pr', 'alpha1_pr', 'alpha2_pr', 
        'Vimp_pr', 'X', 'g', 'Bx', 'By', 'Bz', 'electrostatic_potential', 'site_index_map',
        't', 'Vimp_eta2', 'Vimp_side'
    }
    
    # Add missing parameters with defaults
    default_values = {
        'eps_sw': 0.0, 'alpha1_sw': 0.0, 'alpha2_sw': 0.0, 
        'eps_pr': 0.0, 'alpha1_pr': 0.0, 'alpha2_pr': 0.0,
        'Vimp_pr': 1.0,  # Add missing pristine parameter
        'X': 0.0, 'g': 2.0, 'Bx': 0.0, 'By': 0.0, 'Bz': 0.0,
        'electrostatic_potential': 0.0, 'site_index_map': {},
        't': -2.8, 'Vimp_eta2': 1.0, 'Vimp_side': 0.5
    }
