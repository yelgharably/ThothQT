...
class SCFSolver:
    ...
    def __init__(self, 
        ...
        -----------
        lattice_sites : np.ndarray
            Array of (x,y) coordinates of system sites
        epsilon : float
            Relative permittivity
        max_iterations : int
            Maximum number of SCF iterations
        tolerance : float
            Convergence tolerance for potential difference
        mixing_parameter : float
            Linear mixing parameter (0 < α < 1)
        """
        ...
        # Cache for equilibrium charge density (computed once per system)
        self._equilibrium_charge = None
        
    def linear_mixing(self, phi_new: np.ndarray, phi_old: np.ndarray) -> np.ndarray:
        """
        Apply linear mixing to improve SCF convergence.
        φ_mixed = α * φ_new + (1-α) * φ_old
        """
    ...
    def anderson_mixing(self, phi_new: np.ndarray, phi_old: np.ndarray, 
        ...
        """
        Simple Anderson mixing for better SCF convergence.
        """
    ...
    def compute_charge_density(self, 
        ...
        """
        Compute charge density using NEGF formalism.
        ...
        temperature : float
            Temperature in K
        energy_span : float
            Energy window for integration (smaller is faster; default 0.2 eV)
        n_energy_points : int
            Number of energy points for integration (smaller is faster; default 21)
            
        Returns:
        --------
        np.ndarray
            Local charge density at each site
        """
        try:
            ...
            # NEGF may return charge per orbital; we need charge per site
            if len(charge_density) != self.n_sites:
                if len(charge_density) % self.n_sites == 0:
                    # Multiple orbitals per site - sum over orbitals
                    n_orbs_per_site = len(charge_density) // self.n_sites
                    charge_density = charge_density.reshape(self.n_sites, n_orbs_per_site).sum(axis=1)
                elif len(charge_density) > self.n_sites:
                    # Truncate if too large and not evenly divisible
                    warnings.warn(f"Charge density size {len(charge_density)} doesn't match n_sites {self.n_sites}, truncating")
                    charge_density = charge_density[:self.n_sites]
                else:
                    # Pad if too small
                    warnings.warn(f"Charge density size {len(charge_density)} < n_sites {self.n_sites}, padding with zeros")
    ...
    def compute_equilibrium_charge(self, fsys, params: Dict[str, Any], 
                                  mu_left: float, mu_right: float, 
                                  temperature: float = 0.0) -> np.ndarray:
        """
        Compute equilibrium charge density (at zero bias, zero potential).
        ...
        fsys : kwant.system.FiniteSystem
            Finalized Kwant system
        params : dict
            System parameters (without potential or bias)
        mu_left, mu_right : float
            Chemical potentials (should be equal for equilibrium)
        temperature : float
            Temperature in K
        ...
        """
        if self._equilibrium_charge is not None:
            return self._equilibrium_charge
        
        try:
            # Create equilibrium params with zero potential
            # KWANT requires all parameters from the function signature to be present
            eq_params = params.copy()
            
            # Create zero potential array and site index map
            zero_potential = np.zeros(self.n_sites)
            
            # Build site index map if not already present
            if 'site_index_map' not in eq_params or eq_params['site_index_map'] is None:
                ...
                site_index_map = {site: i for i, site in enumerate(sites_list)}
            else:
                site_index_map = eq_params['site_index_map']
            
            # Set equilibrium params: zero potential with proper site map
            eq_params['electrostatic_potential'] = zero_potential
            eq_params['site_index_map'] = site_index_map
            
            # Compute charge at equilibrium (mu_left = mu_right)
            mu_eq = (mu_left + mu_right) / 2
            
            # Extract matrices and create solver
            H_device, H_leads, V_couplings = extract_kwant_matrices(
                fsys, mu_eq, leads_kwant_supported=None, params=eq_params
            )
            negf = NEGFSolver(H_device, H_leads, V_couplings)
            
            # Use narrow energy window around Fermi level for equilibrium
            charge_eq = negf.charge_density_at_bias(
                mu_eq, mu_eq, temperature=temperature,
                E_span=0.5, n_points=101
            )
            
            # Handle multi-orbital case: sum over orbitals to get per-site charge
            if len(charge_eq) != self.n_sites:
                if len(charge_eq) % self.n_sites == 0:
                    n_orbs_per_site = len(charge_eq) // self.n_sites
                    charge_eq = charge_eq.reshape(self.n_sites, n_orbs_per_site).sum(axis=1)
                else:
                    warnings.warn(f"Equilibrium charge size {len(charge_eq)} doesn't match n_sites {self.n_sites}")
                    # Truncate as fallback
                    charge_eq = charge_eq[:self.n_sites]
            
            # Cache result
            self._equilibrium_charge = charge_eq
            return charge_eq
            
        except Exception as e:
            warnings.warn(f"Failed to compute equilibrium charge: {e}")
            return np.zeros(self.n_sites)
    ...
    def scf_loop(self,
        ...
        for iteration in range(self.max_iterations):
            ...
            try:
                ...
                charge_density_total = self.compute_charge_density(
                    fsys, current_params, mu_left, mu_right, temperature
                )
                
                # Compute equilibrium charge (cached after first call)
                if iteration == 0:
                    if verbose:
                        print(f"  Iter 1: computing equilibrium charge for excess charge calculation...")
                    charge_eq = self.compute_equilibrium_charge(
                        fsys, initial_params, mu_left, mu_right, temperature
                    )
                    if verbose:
                        if charge_eq is not None and not np.all(charge_eq == 0):
                            print(f"  Iter 1: equilibrium charge range: [{np.min(charge_eq):.3e}, {np.max(charge_eq):.3e}]")
                        else:
                            print(f"  Iter 1: WARNING - equilibrium charge is zero/None, using total charge density!")
                else:
                    charge_eq = self._equilibrium_charge
                
                # Use EXCESS charge density for Poisson equation
                # This is the induced charge from bias, not total electron density
                if charge_eq is not None and not np.all(charge_eq == 0):
                    charge_density = charge_density_total - charge_eq
                    if verbose and iteration == 0:
                        print(f"  Iter 1: using EXCESS charge = total - equilibrium")
                        print(f"  Iter 1: excess charge range: [{np.min(charge_density):.3e}, {np.max(charge_density):.3e}]")
                else:
                    charge_density = charge_density_total
                    if verbose and iteration == 0:
                        print(f"  Iter 1: WARNING - using TOTAL charge (equilibrium failed)")
            ...
            try:
                ...
                # Allow selecting Poisson method via params: 'auto'|'fem'|'graph'|'grid'
                method = current_params.get('poisson_method', None)
                phi_new = self.poisson_solver.solve_graphene(
                    charge_density, bias_voltage, (mu_left, mu_right), method=method
                )
            ...
            if compute_current_each_iter:
                try:
                    ...
                    # Use a moderate number of energy points for speed during iterations
...
def compute_finite_bias_current(fsys, 
    ...
    """
    Compute finite bias current using energy integration:
    I(V) = (2e/h) ∫ T(E,V) [f_L(E) - f_R(E)] dE
    ...
    fsys : kwant.system.FiniteSystem
        Finalized Kwant system
    params : dict
        System parameters (including converged potential)
    bias_voltage : float
        Applied bias voltage in V
    temperature : float
        Temperature in K
    energy_window : float
        Energy integration window around Fermi level (eV)
    n_energy_points : int
    ...
    --------
    Tuple[float, float]
        (current in A, average transmission)
    """
    ...
    if verbose:
        ...
        print(f"  ⚠️  **PHYSICS CHECK**: Current magnitude = {current:.2e} A")
        if abs(current) > 1e9:
            ...
            print(f"      ✅ Magnitude reasonable for molecular/nanoelectronics device")
...
def scf_conductance(fsys, 
    ...
    """
    Compute conductance with self-consistent electrostatics.
    ...
    Tuple[float, float, bool]
        (conductance, transmission, scf_converged)
    """
    # Initialize SCF solver
    scf = SCFSolver(
